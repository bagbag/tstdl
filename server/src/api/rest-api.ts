import { createErrorResponse, createResultResponse } from '@common-ts/base/api';
...
export class RestApi {
  ...
  registerGetRoute<TParameters, TResult>(path: string, validator: ValidationFunction, handler: FunctionHandler<TParameters, TResult>): void {
  ...
  registerPostRoute<TParameters, TResult>(path: string, validator: ValidationFunction, handler: FunctionHandler<TParameters, TResult>): void {
  ...
  private async handle<TBody, TResult>(context: Koa.Context, validator: ValidationFunction, handler: FunctionHandler<TBody, TResult>): Promise<void> {
    ...
    if (validationResult.valid) {
      try {
        const result = await handler(parameters as TBody);
      ...
      response.status = 400;
      response.body = createErrorResponse('invalid parameters', validationResult.error);
    }
  }
}
