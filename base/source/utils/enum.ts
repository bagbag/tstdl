...
function _enumEntries<T extends EnumerationObject>(enumeration: T): EnumEntry<T>[] {
  return Object.entries(enumeration)
    .filter((entry): entry is EnumEntry<T> => Number.isNaN(Number(entry[0])));
}

function _enumKeys<T extends EnumerationObject>(enumeration: T): EnumKey<T>[] {
  return _enumEntries(enumeration).map((entry) => entry[0]);
}

function _enumValues<T extends EnumerationObject>(enumeration: T): EnumValue<T>[] {
